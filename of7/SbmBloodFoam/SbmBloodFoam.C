...
(RBCs) and other deformable particle suspensions in confined geometries. A lift force term "LiftF" and other hemorheology features
...
//Volume fraction (or hematocrit) is defined as "c"
...
LiftF = (1.0*((2.0*sqr(a))/(9.0*nu0))*fc* fv * ((3.0* nu0 *gammadot)/(4.0 * 3.14159))*c *Foam::vector(0,1,0))/( pow( dimensionedScalar("one",dimensionSet(0,-1,0,0,0,0,0),scalar(1.0))*(mesh.C().component(1)+h0), beta) );
